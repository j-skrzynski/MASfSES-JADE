...
public class TimeHandlingBehaviour extends TickerBehaviour {
    ...
    public TimeHandlingBehaviour(
            StockExchangeAgent a,
            long period,
            long millisecondsPerSession,
            long expirationUpdateInterval
    ) {
        super(a, period);
        agent = a;
        milliseconds = period;
    ...
    protected void onTick() {
        ...
        if (agent.getStockExchange().getMillisecondsSinceStart() > 0 &&
                agent.getStockExchange().getMillisecondsSinceStart() % expirationUpdateInterval == 0) {

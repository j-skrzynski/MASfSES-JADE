...
public class PlaceOrder extends BrokerCommand{
    ...
    public PlaceOrder setExpirationSpecification(String specification) {
        ((AwaitingOrder)this.arguments.get(0)).getOrder().setExpirationSecpification(specification);
        return this;
    }
    ...
    public static PlaceOrder market_order(String shortName, OrderType type, Double price, Long quantity){
    ...
    public static PlaceOrder limitless_order(String shortName, OrderType type, Long quantity){
        StockSymbol symbol = StockDictionary.getStockIdByShortName(shortName);
    ...
    public static PlaceOrder awaiting_market_order(String shortName, OrderType type, Double price, Long quantity, Double activationPrice){
        StockSymbol symbol = StockDictionary.getStockIdByShortName(shortName);
        Order order = new Order(symbol, type, price, quantity);
    ...
    public static PlaceOrder awaiting_limitless_market_order(String shortName, OrderType type, Long quantity, Double activationPrice){
        StockSymbol symbol = StockDictionary.getStockIdByShortName(shortName);
        Order order = new Order(symbol, type, null, quantity);
        AwaitingOrder aw_order = new AwaitingOrder(order, true,activationPrice);
        return new PlaceOrder(aw_order);
